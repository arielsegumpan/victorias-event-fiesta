    ...
    <div class="px-4 py-10 mx-auto sm:px-6 lg:px-8 lg:py-14">
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div
                ...
                @if(auth()->check())
                <form wire:submit.prevent="submitReview" x-data="dropzoneHandler()"
                    x-on:drop.prevent="dropFiles($event)" x-on:dragover.prevent enctype="multipart/form-data"
                    class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="text-center">
                            ...
                            <div class="mt-2 flex justify-center items-center space-x-2">
                                ...
                                <button type="button" wire:click="$set('rating', {{ $rate }})"
                    ...
                    <div>
                        ...
                        <textarea wire:model.blur="review" rows="5"
                    ...
                    remove(index) {
                        ...
                        <!-- Dropzone -->
                        <div class="w-full p-6 border-2 border-dashed border-neutral-800 dark:border-neutral-600 rounded-lg text-center cursor-pointer bg-white hover:bg-neutral-200 dark:hover:bg-neutral-700 transition dark:bg-neutral-800"
                            :class="{ 'border-orange-500': dragging }" x-ref="dropzone" x-on:dragenter="dragging = true"
                            x-on:dragleave="dragging = false" x-on:drop="dragging = false"
                            x-on:click="$refs.fileInput.click()">
                            <input type="file" wire:model="review_images" multiple accept="image/*" class="hidden"
                                x-ref="fileInput"
                                @change="$refs.fileInput.dispatchEvent(new Event('input', { bubbles: true }))">
                        ...
                        <div class="flex flex-wrap gap-2 mt-4">
                            ...
                            <div class="relative group w-20 h-20">
                                <img src="{{ $img->temporaryUrl() }}"
                                    class="w-full h-full object-cover border dark:border-gray-200 border-gray-400 rounded">
                                <!-- Close Icon -->
                                <button type="button"
                        ...
                        <div x-show="isUploading" class="mt-2">
                            ...
                            <p class="text-xs mt-1 text-orange-600">{{ __('Uploading...') }} <span
                                    x-text="progress"></span>{{ __('%') }}</p>
                        </div>
                    ...
                    <button wire:loading.attr="disabled" type="submit"
                        ...
                        <svg class="shrink-0 size-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="flex flex-row items-center justify-center">
                    ...
                    <div class="text-gray-800 dark:text-neutral-400">
                        {{ __('Please') }} <a
                            class="mx-4 font-medium text-orange-600 decoration-2 hover:underline focus:outline-none focus:underline dark:text-orange-500"
                            href="{{ route('filament.auth.auth.login') }}">{{ __('login') }}</a> {{ __('to write a
                        review') }}
                    </div>
                </div>
                </p>
